...
开发者账号后台支持新增/编辑账号、配置账号级 connector、绑定/解绑 App、维护 App 商店标识，并在账号上下文中同步版本说明和文字类商店元数据。商店元数据的关键词、宣传文本和描述支持多语言编辑，支持语言从 connector 拉取；缺少翻译时默认使用当前语言内容填充。商店同步页面进入时会自动预检查；相同账号、App、平台、版本、语言和操作的预检查结果缓存 5 分钟。
