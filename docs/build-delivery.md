...
每个 Runner 当前固定单并发（`capacity=1`）。任务分配同时匹配 Runner 声明的平台能力和应用构建配置
中的标签；不匹配的 Runner 不会领取任务。
...
应用构建配置 + 创建构建任务
        |
        v
queued -> Runner 按平台和标签领取 -> assigned / building
        |
        v
...
在管理后台的应用详情中，开发环境与线上环境分别维护一份构建配置。创建任务时需要明确：
...
安装后可在管理后台的“构建节点”页面确认节点状态、标签、版本、能力、最近心跳和当前任务。
...
Webhook URL 和加签密钥只从服务端环境读取，不会由管理 API 返回，也不会写入数据库或日志。投递失败
